...
void Object::CreateAllSRV()
{
	if (m_material->m_baseTexture)
	{
		m_material->CreateSRV(m_material->m_baseTexture);
	}

	if (m_material->m_materialConstantData.usePBR)
	{
		if (m_material->m_materialConstantData.useNormalMap)
		{
			m_material->CreateSRV(m_material->m_normalTexture);
		}
		else
		{
			// 주소값 증가
			int index = OBJ_HEAP->GetSRVIndex();
		}

		if (m_material->m_materialConstantData.useAoMap)
		{
			m_material->CreateSRV(m_material->m_aoTexture);
		}
		else
		{
			// 주소값 증가
		...
		if (m_material->m_materialConstantData.useMetallicMap)
		{
			m_material->CreateSRV(m_material->m_metallicTexture);
		}
		else
		{
			// 주소값 증가
		...
		if (m_material->m_materialConstantData.useRoughnessMap)
		{
			m_material->CreateSRV(m_material->m_roughnessTexture);
		}
		else
		{
			// 주소값 증가
		...
		if (m_material->m_materialConstantData.useEmissionMap)
		{
			m_material->CreateSRV(m_material->m_emissionTexture);
		}
		else
		{
			// 주소값 증가
